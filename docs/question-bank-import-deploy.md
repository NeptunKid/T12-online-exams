...
本步骤包含四份已确认的 Excel 题库：萃取原理、消防基础、IT 基础、咖啡基础知识。两组待人工审核的 PDF 和咖啡师招聘笔试不在导入范围内。
...
`--only coffee` 确保本次只导入咖啡基础知识，不核对或修改已经上线的考试。若需完整初始化四份结构化题库，可移除该参数。若只需给单个员工授权，可改用 `--union-id '<钉钉 unionId>' --user-name '<员工姓名>'`；真实身份值不得写入 Git。导入脚本会校验题库数量、题目内容、总分、考试时长和 85% 通过线；重复执行不会重复创建，已有内容不一致时会整笔回滚并报错。
...
sudo -u postgres psql -d t12_exams -c \
  "SELECT e.id, e.title, e.status, count(eq.question_id) AS questions, e.total_score, e.pass_score \
   ...
   WHERE e.id IN ('exam-extraction-principle','exam-fire-basics','exam-it-basics','exam-coffee-basics') \
