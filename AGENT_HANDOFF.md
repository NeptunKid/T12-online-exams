...
本轮已提交：题目图片资源提交 `5f42217`
当前未提交：试卷/题库 `.t12backup` 手动导出、一键事务导入、管理员 UI、测试和文档
...
10. 可移植备份尚未连接真实 PostgreSQL 做浏览器下载、上传和大图片包验收；定期自动备份、保留策略及外部存储目标尚未实现。
...
5. 在已完成可移植导入导出基础上，增加定期自动备份、保留策略、外部存储适配、监控告警和恢复演练。
