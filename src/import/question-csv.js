...
function allowedImageUrl(value, allowedImageHosts) {
  if (value.startsWith("resource:")) return /^resource:[A-Za-z0-9_-]+$/.test(value);
  try {
...
function previewQuestionCsv(text, options = {}) {
  ...
  rows.slice(1).forEach((rawRow, offset) => {
    ...
    const lastOption = optionValues.map(Boolean).lastIndexOf(true);
    ...
    if (lastOption >= 0 && optionValues.slice(0, lastOption + 1).some((value) => !value)) {
    ...
    for (const imageUrl of imageUrls) {
      if (!allowedImageUrl(imageUrl, allowedImageHosts)) {
